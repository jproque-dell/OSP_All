...
EOF
		...
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT f5_vtep_folder None
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT f5_vtep_selfip_name None
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT f5_global_routed_mode False
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT f5_snat_addresses_per_subnet 0
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT icontrol_password fedex123
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT f5_external_physical_mappings default:apic_trunk1:True

		echo -e "Configure Neutron LBaaS service \n"
		/bin/cp -fv /etc/neutron/neutron_lbaas.conf /etc/neutron/neutron_lbaas.conf.$(date +%m%d%y)
		crudini --set /etc/neutron/neutron_lbaas.conf service_providers service_provider LOADBALANCERV2:F5Networks:neutron_lbaas.drivers.f5.driver_v2.F5LBaaSV2Driver:default

		echo -e "Configure Neutron File \n"
		/bin/cp -fv /etc/neutron/neutron.conf /etc/neutron/neutron.conf.$(date +%m%d%y)
		crudini --set /etc/neutron/neutron.conf DEFAULT service_plugins cisco_apic_l3,metering,neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2
		...
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT icontrol_hostname __F5_ICONTROL_HOSTNAME__
		crudini --set /etc/neutron/services/f5/f5-openstack-agent.ini DEFAULT environment_prefix __F5_ENV_PREFIX__
