...
if [ -d ${CA_DIR} ]; then
	if [ -f ${CA_DIR}/corp.pem ]; then
		echo "Certificate already present. Skipping..."
	else
		echo "Instaling Certificate as ${CA_DIR}/corp.pem ..."
cat >>  ${CA_DIR}/corp.pem << EOF
-----BEGIN CERTIFICATE-----
...
EOF
	...
	/bin/update-ca-trust
	#
	/bin/rm -fv /etc/pki/tls/certs/corp.crt
	ln -s ${CA_DIR}/corp.pem /etc/pki/tls/certs/corp.crt
	#
	fi
fi
