#!/bin/bash
#
set -x ; VLOG=/var/log/ospd/pre_deploy-bz1406417_ceph_rgw.log ; exec &> >(tee -a "${VLOG}")
my_node_role=$(cat /var/log/ospd/node_role)
patch_file=/var/tmp/bz1406417_ceph_rgw.patch
#
case ${my_node_role} in
	*)
		if [ -f /usr/lib/python2.7/site-packages/ceph_rgw/storage/ceph.py ]; then
cat > ${patch_file} << EOF
*** /usr/lib/python2.7/site-packages/ceph_rgw/storage/ceph.py.orig       2017-03-10 21:46:05.000000000 +0000
--- /usr/lib/python2.7/site-packages/ceph_rgw/storage/ceph.py    2017-04-12 13:50:13.295146413 +0000
***************
*** 50,55 ****
--- 50,56 ----   
...
EOF
			if [ -f ${patch_file} ]; then
				sudo patch -p0 < \
				${patch_file} \
				/usr/lib/python2.7/site-packages/ceph_rgw/storage/ceph.py
			fi
		fi
  ;;
esac
